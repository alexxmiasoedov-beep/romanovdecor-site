#!/usr/bin/env python3
# Прайс для менеджера под брендом UNICORE. Цены под ключ за м² в евро.
# Без курса и себестоимости — этот лист можно давать в руки и показывать клиенту.
#
# Копия build_memo_manager.py: расценки, формулы и вёрстка совпадают строка
# в строку, отличается только шапка. Оба файла живут параллельно, пока идёт
# переход на новое имя. Цены правятся в обоих или выносятся в общий модуль.
#
#   python3 build_memo_unicore.py && node topdf.js memo-unicore "Прайс-UNICORE"
import os
...
FIX = [('Дверь 700×2000', '€63 за <u>сторону</u> полотна + материал по площади'),
...
logo = ('<svg width="44" height="44" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">'
        ...
        'text-anchor="middle" dominant-baseline="middle">U</text></svg>')
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;border-bottom:2px solid #b8965a;padding-bottom:8px">
  <div style="display:flex;align-items:center;gap:12px">{logo}
    <div><div style="font-size:20px;letter-spacing:4px;font-weight:600;text-transform:uppercase;line-height:1.1">UNI<span style="color:#b8965a">CORE</span></div>
    <div style="font-size:9px;color:#888;margin-top:4px;letter-spacing:1px;text-transform:uppercase">производство микроцемента · немецкие компоненты</div></div></div>
...
<h3 style="{H3}">Границы и объём</h3>
{two_col(RULES, BEIGE)}
...
open(SC + '/kp/memo-unicore.html', 'w').write(html)
for a in AREAS:
