#!/usr/bin/env python3
# КП — ул. Лосика 53, заказчик 7komnat. Стены + двери, микроцемент.
# Объёмы берутся из файла замеров, цены — из «Памятки по ценам» (шкала по слоям).
import openpyxl, os
...
def pairs(c1, c2, r1, r2):
    out = []
    for r in range(r1, r2 + 1):
        a, b = ws.cell(r, c1).value, ws.cell(r, c2).value
        if isinstance(a, (int, float)) and isinstance(b, (int, float)):
            out.append((a, b))
    return out

def measure(ps, thr=0.5):
    """материал — вся площадь; работа — стороны от 0,5 м в м², меньше — в м.п."""
    m2 = sq = lin = 0.0
    for a, b in ps:
        m2 += a * b
...
def scale(total_m2, work_units):
    mat = PACK * 40 / 64          # пакет делим в пропорции базовых ставок 40:24
    wrk = PACK * 24 / 64
...
rows = ''
TM = TW = 0.0
for name, m2, sq, lin in zones:
    m = m2 * EFF_MAT; w = (sq + lin) * EFF_WRK
    TM += m; TW += w
    vol = ' + '.join(([f'{a2(sq)} м²'] if sq else []) + ([f'{a2(lin)} м.п.<sup>*</sup>'] if lin else []))
    rows += row(name, f'{a2(m2)} м²', u(EFF_MAT), u(m), vol, u(EFF_WRK), u(w), u(m + w))
...
open(SC + '/kp/kp-losika.html', 'w').write(html)
for n, m2, sq, lin in zones:
    print(f'{n:16s} мат {m2:7.2f} м²   раб {sq:7.2f} м² + {lin:6.2f} м.п.')
print(f'{"стены":16s} мат {W_M2:7.2f} м²   раб {W_UN:7.2f} ед.')
print(f'{"двери":16s} мат {D_M2:7.2f} м²   {ND} шт')
print(f'объём для шкалы {a2(TOT_M2)} м² | эфф. материал {u(EFF_MAT)}/м² | эфф. работа {u(EFF_WRK)}')
